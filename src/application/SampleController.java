...
public class SampleController {
	  ...
	  @FXML
	    void ReadFromInterface(ActionEvent event) {//reading from interface
		  ...
		  int temp = 0;
			...
			for (int i = 0; i < DIMENTION; i++) {
				for (int j = 0; j < DIMENTION; j++) {
					if(TF.get(DIMENTION*i + j).getText().equals(""))B[i][j]=0;
					else B[i][j] = Integer.valueOf(TF.get(DIMENTION*i + j).getText());
		    		temp = B[i][j];
	...
	public static boolean isValid(Cell current, int num) {
		...
		if(Row.get(current.getRow()).get(num) == true)return false;
		//check if it exists in this column
		if(Col.get(current.getCol()).get(num) == true)return false;
		//check the 3x3 grid
		if(Grid.get(getGridNum(current.getRow(), current.getCol())).get(num) == true)return false;
		//if non of them have it, then the number is valid.
		return true;
	}
	
	public static boolean isSolvable(int i, int j, int temp) {
		if(Row.get(i).get(temp) || Col.get(j).get(temp) || Grid.get(getGridNum(i, j)).get(temp))return false;
		Row.get(i).flip(temp);
		Col.get(j).flip(temp);
	...
	public static void styleCell(StackPane sp, int i, int j, int type) {
		 String color= "";
			if(type == 0)color = "ffffff";
			else if(type == 1)color = "feffa5";
			else if(type == 2)color = "bdff8f";
			else color = "ffa48f";
			
		if(j == 3 || j == 6)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 1 1 1 4; -fx-border-color: black; -fx-background-color: #"+color+";");
		else if(i == 3 || i == 6)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 4 1 1 1; -fx-border-color: black; -fx-background-color: #"+color+";");
		else sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 1; -fx-border-color: black; -fx-background-color: #"+color+";");
		if(i == 3 && j == 3)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 4 1 1 4; -fx-border-color: black; -fx-background-color: #"+color+";");
		if(i == 3 && j == 6)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 4 1 1 4; -fx-border-color: black; -fx-background-color: #"+color+";");
		if(i == 6 && j == 3)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 4 1 1 4; -fx-border-color: black; -fx-background-color: #"+color+";");
		if(i == 6 && j == 6)sp.setStyle("-fx-border-style: solid solid solid solid; -fx-border-width: 4 1 1 4; -fx-border-color: black; -fx-background-color: #"+color+";");
	}
